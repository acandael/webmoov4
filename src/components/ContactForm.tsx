...
const ContactForm = ({ align = "left", product }: ContactFormProps) => {
	...
	return (
		<div className={styles.formSection}>
			<div className={`${styles.heading} ${align === "center" ? styles.centerAlign : ""}`}>
				...
				<div className={styles.formGrid}>
					<div className={styles.formGroup}>
						...
						{form.formState.errors.name && (
							<span className={styles.errorMessage}>{form.formState.errors.name.message}</span>
						)}
					</div>
					<div className={styles.formGroup}>
						...
						{form.formState.errors.email && (
							<span className={styles.errorMessage}>{form.formState.errors.email.message}</span>
						)}
					</div>
					<div className={`${styles.formGroup} ${styles.fullWidth}`}>
						<label htmlFor="phone">Telefoon (optioneel)</label>
						<input type="tel" id="phone" {...form.register("phone")} placeholder="0498163250" />
						{form.formState.errors.phone && (
							<span className={styles.errorMessage}>{form.formState.errors.phone.message}</span>
						)}
					</div>
					<div className={`${styles.formGroup} ${styles.fullWidth}`}>
						...
						{form.formState.errors.message && (
							<span className={styles.errorMessage}>{form.formState.errors.message.message}</span>
						)}
					</div>
				</div>
